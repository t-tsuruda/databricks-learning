...
export async function createLesson(courseId: string, formData: FormData) {
  ...
  const lesson = await prisma.lesson.create({ data: { ...values, courseId } });
...
export async function deleteLesson(courseId: string, formData: FormData) {
  await requireAdminSession();
...
export async function updateLessonQuizzes(courseId: string, lessonId: string, formData: FormData) {
  await requireAdminSession();
  ...
  try {
    ...
    throw new Error("確認問題の形式が正しくありません。");
  }
